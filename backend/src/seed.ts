...
async function seed() {
  ...
  const alumnosData = [
    {
      email: 'alumno@frlp.utn.edu.ar',
      legajo: 'UTN-001',
      nombre: 'Juan',
      apellido: 'García',
    },
    {
      email: 'alumno2@frlp.utn.edu.ar',
      legajo: 'UTN-002',
      nombre: 'María',
      apellido: 'Pérez',
    },
    {
      email: 'alumno3@frlp.utn.edu.ar',
      legajo: 'UTN-003',
      nombre: 'Lucas',
      apellido: 'Rodríguez',
    },
  ...
  const proyectosData = [
    ...
    },
    {
      ...
      descripcion:
        'Optimización de despliegue de microservicios en clusters locales usando Docker y Kubernetes para investigación académica.',
      laboratorio: 'LIDIC',
      skills: ['Python', 'SQL'],
  ...
  const lidicProjs = spawnedProyectos.filter(
    (p) => p.laboratorio.nombre === 'LIDIC',
  );
